...
class kitti_set(torch.utils.data.Dataset):
    ...
    def __init__(self, path, split) -> None:
        ...
        self.VELOPATH = op.join(cfg.SRCPATH, 'data_object_velodyne', split)
        self.IMGPATH = op.join(cfg.SRCPATH, 'data_object_image_2', split)
        self.LABELPATH = op.join(cfg.SRCPATH, 'data_object_label_2', split)
        self.CALPATH = op.join(cfg.SRCPATH, 'data_object_calib', split)
