...
class kitti_set(torch.utils.data.Dataset):
    ...
    def __init__(self, path, split) -> None:
        ...
        self.VELOPATH = op.join(cfg.SRCPATH, 'data_object_velodyne', split, 'velodyne')
        self.IMGPATH = op.join(cfg.SRCPATH, 'data_object_image_2', split, 'image_2')
        self.LABELPATH = op.join(cfg.SRCPATH, 'data_object_label_2', 'train')
        self.CALPATH = op.join(cfg.SRCPATH, 'data_object_calib', split, 'calib')
    ...
    def __getitem__(self, idx):
        ...
        points = np.fromfile(op.join(self.VELOPATH, lidar_file), dtype=np.float32).reshape(-1, 4)
        intensity = points[:, 3]
        points = points[:, 0:3]
        ...
        img = Image.open(op.join(self.IMGPATH, img_file))
